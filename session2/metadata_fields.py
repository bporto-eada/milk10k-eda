...
def categorical_link(df, col):
    ...
    x = d[col].map(lambda v: MISSING if pd.isna(v) else str(v))
    frequency = x.map(x.value_counts())
    x = x.where(frequency >= MIN_GROUP, RARE)
...
def numeric_link(df, col):
    ...
    medians = d.groupby(LABEL)[col].median().round(1).to_dict()
    return {"field": col, "test": "Kruskal-Wallis", "strength": eta, "p": p, "medians": medians}
...
def run(df, say):
    ...
    for _, row in summary.iterrows():
        ...
        if row["kind"] in ("categorical", "boolean"):
            ...
            results.append(result)
            say(f"{col} median per class: {result['medians']}")

    chart_overview(results)
    ranking = pd.DataFrame([{k: v for k, v in r.items() if k not in ("table", "medians")} for r in results])
